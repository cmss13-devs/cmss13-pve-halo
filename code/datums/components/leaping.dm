...
/datum/component/leaping/proc/activate_leap(direction)
	...
	addtimer(CALLBACK(src, PROC_REF(end_leap), leaper, effective_leaper_allow_pass_flags), 0.2 SECONDS)
	ASYNC
		for(var/i=1 to leap_range)
			step(leaper, direction)
			sleep(1)

/datum/component/leaping/proc/end_leap(atom/leaper, effective_leaper_allow_pass_flags)
	var/mob/living/living_leaper
	if(isliving(parent))
		living_leaper = leaper
		if(living_leaper.body_position != LYING_DOWN)
