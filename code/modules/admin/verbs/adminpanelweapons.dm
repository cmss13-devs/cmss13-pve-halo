/client/proc/adminpanelweapons()
	...
	// HALO PVE EDIT - START - Rewording to fit better, PD versus DEW is pointless
	if(intercept_chance)
		addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(shipwide_ai_announcement), "ATTENTION: TRACKING TARGET[quantity > 1 ? "S" : ""], EMERGENCY THRUSTERS PRIMED. ATTEMPTING TO EVADE." , MAIN_AI_SYSTEM, 'sound/effects/supercapacitors_charging.ogg'), (hit_eta - 4) SECONDS)
	// HALO PVE EDIT - END
