...
/datum/human_ai_spawner_menu/ui_static_data(mob/user)
	...
	data["selectable_factions"] = FACTION_LIST_HALO + "Normal Hive" + "Zombie"
	data["equipment_settings"] = list("Full Equipment", "No Weapons", "Birthday Suit")
	data["species_settings"] = PLAYER_SYNTHS + SPECIES_HUMAN + SPECIES_SANGHEILI + SPECIES_UNGGOY + SPECIES_ZOMBIE
	return data
...
/datum/human_ai_spawner_menu/proc/SpawnHuman(mob/user, params, atom/object)
	...
	if(selected_equipment == "No Weapons")
		...
		if(gotten_path::faction in FACTION_LIST_UNSC)
...
//					/obj/item/clothing/head/helmet/construction,
	...
	if(selected_faction != faction_of_preset)
		...
		if(selected_faction in FACTION_LIST_UNSC)
			ai_human.faction_group = FACTION_LIST_UNSC
			if(faction_tags)
				faction_tags.faction_group = FACTION_LIST_UNSC
		else
